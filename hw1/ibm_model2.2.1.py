...
# IBM Model 2
# Heuristic: When two alignments tie, choose the one that has the closer relative position.
# Calculate argmax_i and argmax_j, and then intersect.
...
# Print output
outfile = open("output-model2.2.txt", "w")
for (f, e) in bitext:
	list_f_e = []
	for (i, f_i) in enumerate(f):
		i_pos = 1.0 * i / len(f)
	...
	for (j, e_j) in enumerate(e):
		...
		for (i, f_i) in enumerate(f):	
			i_pos = 1.0 * i / len(f)
			value = q[(j,i,len(e),len(f))]*t[(f_i,e_j)]
			if value > max_f_i_value or ( value == max_f_i_value and abs(i_pos-j_pos) < abs(max_pos-j_pos) ):
				max_f_i_value = value
				max_i = i
				max_pos = i_pos
		list_e_f += [ (max_i,j) ]
